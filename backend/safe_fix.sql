-- SAFE FIX - Set time_slot_id to NULL instead of deleting appointments

-- Step 1: Check the problematic appointment
SELECT 'Problematic appointment details:' as info;
SELECT 
    a.id,
    a.patient_id,
    a.time_slot_id,
    a.status,
    a.appointment_date,
    u.full_name as patient_name
FROM appointments a
JOIN users u ON a.patient_id = u.id
WHERE a.time_slot_id = '94ce129f-37de-4460-a94c-bace3c8f2c84';

-- Step 2: Set time_slot_id to NULL for all appointments with invalid time slots
-- This breaks the foreign key relationship without deleting the appointment
UPDATE appointments 
SET 
    time_slot_id = NULL,
    status = 'cancelled',
    cancellation_reason = 'System cleanup: Invalid time slot removed, please reschedule',
    updated_at = NOW()
WHERE time_slot_id IN (
    SELECT ts.id 
    FROM time_slots ts
    JOIN work_schedules ws ON ts.work_schedule_id = ws.id
    JOIN shifts s ON ws.shift_id = s.id
    WHERE NOT (
        (s.start_time >= '08:00:00' AND s.end_time <= '12:00:00') OR
        (s.start_time >= '13:30:00' AND s.end_time <= '17:30:00')
    )
);

-- Step 3: Verify no appointments reference invalid time slots
SELECT 'Verification - should be 0:' as info;
SELECT COUNT(*) as remaining_references
FROM appointments a
JOIN time_slots ts ON a.time_slot_id = ts.id
JOIN work_schedules ws ON ts.work_schedule_id = ws.id
JOIN shifts s ON ws.shift_id = s.id
WHERE NOT (
    (s.start_time >= '08:00:00' AND s.end_time <= '12:00:00') OR
    (s.start_time >= '13:30:00' AND s.end_time <= '17:30:00')
);

-- Step 4: Now delete time slots (should work)
DELETE FROM time_slots 
WHERE work_schedule_id IN (
    SELECT ws.id FROM work_schedules ws
    JOIN shifts s ON ws.shift_id = s.id
    WHERE NOT (
        (s.start_time >= '08:00:00' AND s.end_time <= '12:00:00') OR
        (s.start_time >= '13:30:00' AND s.end_time <= '17:30:00')
    )
);

-- Step 5: Delete work schedules
DELETE FROM work_schedules 
WHERE shift_id IN (
    SELECT id FROM shifts 
    WHERE NOT (
        (start_time >= '08:00:00' AND end_time <= '12:00:00') OR
        (start_time >= '13:30:00' AND end_time <= '17:30:00')
    )
);

-- Step 6: Delete invalid shifts
DELETE FROM shifts 
WHERE NOT (
    (start_time >= '08:00:00' AND end_time <= '12:00:00') OR
    (start_time >= '13:30:00' AND end_time <= '17:30:00')
);

-- Step 7: Create standard shifts
INSERT INTO shifts (name, start_time, end_time, created_at)
SELECT 'Morning', '08:00:00', '12:00:00', NOW()
WHERE NOT EXISTS (SELECT 1 FROM shifts WHERE name = 'Morning' AND start_time = '08:00:00');

INSERT INTO shifts (name, start_time, end_time, created_at)
SELECT 'Afternoon', '13:30:00', '17:30:00', NOW()
WHERE NOT EXISTS (SELECT 1 FROM shifts WHERE name = 'Afternoon' AND start_time = '13:30:00');

-- Step 8: Show results
SELECT 'Final shifts:' as info;
SELECT id, name, start_time, end_time FROM shifts ORDER BY start_time;

SELECT 'Appointments with NULL time_slot_id (cancelled):' as info;
SELECT COUNT(*) as cancelled_appointments
FROM appointments 
WHERE time_slot_id IS NULL 
AND cancellation_reason LIKE '%System cleanup%';

-- Step 9: Final verification
SELECT 'Should be 0 - invalid time slots remaining:' as info;
SELECT COUNT(*) as invalid_remaining
FROM time_slots ts
JOIN work_schedules ws ON ts.work_schedule_id = ws.id
JOIN shifts s ON ws.shift_id = s.id
WHERE NOT (
    (s.start_time >= '08:00:00' AND s.end_time <= '12:00:00') OR
    (s.start_time >= '13:30:00' AND s.end_time <= '17:30:00')
);