...
NINEROUTER_MODEL = os.getenv("NINEROUTER_MODEL", "gemini-2.0-flash") # Default to gemini via 9router

print(f"INFO: 9router key set: {'YES' if NINEROUTER_API_KEY else 'NO'}")
print(f"INFO: Gemini key set: {'YES' if GEMINI_API_KEY else 'NO'}")
...
def _generate_text_ninerouter(prompt: str) -> str:
    """Call 9router API (OpenAI compatible) as a stable alternative."""
    import requests
    ...
    try:
        headers = {
            ...
            "HTTP-Referer": "http://localhost:3000", # Required by some routers
            "X-Title": "Dental App Chatbot"
        }
        ...
        url = f"{NINEROUTER_BASE_URL}/chat/completions"
        ...
        if response.status_code != 200:
            print(f"9router error {response.status_code}: {response.text[:200]}")
            return ""
        ...
        print(f"DEBUG: 9router exception: {e}")
        return ""


def _generate_text(prompt: str) -> str:
    """LLM call wrapper with 9router preference and Gemini fallback."""
    if os.getenv("MOCK_GEMINI", "") == "1":
        return "[MOCK] Xin chao ban. Day la cau tra loi mau de test pipeline."

    # 1. Try 9router if configured (User preference for stability)
    if NINEROUTER_API_KEY:
        text = _generate_text_ninerouter(prompt)
        # If 9router was tried, we return its result (even if empty) 
        # to avoid hitting Gemini quota if the user intentionally uses 9router.
        if text:
            return text
        elif NINEROUTER_API_KEY: 
            # If it returned empty but key is set, maybe it's a model issue
            return "9router tra ve ket qua rong. Vui long kiem tra model hoac balance."

    # 2. Fallback to direct Gemini if 9router is NOT configured
    if GEMINI_API_KEY:
        try:
...
def summarize(messages: list[str], ml_label: str) -> str:
    """Summarize chat messages for doctor review using Gemini."""
    if not messages:
        return "Khong co tin nhan de tom tat."
    
    chat_text = "\n".join([f"- {m}" for m in messages])
    prompt = f"""
    Ban la mot tro ly nha khoa. Hay tom tat ngan gon (khoang 2-3 cau) cac trieu chung va thong tin 
    ma benh nhan da cung cap trong cac tin nhan duoi day. 
    Luu y: Benh nhan dang duoc chan doan so bo la: {ml_label}.
    
    Tin nhan cua benh nhan:
    {chat_text}
    
    Yeu cau:
    - Tom tat dung cac trieu chung chinh.
    - Ngon ngu chuyen nghiep, ngan gon.
    - Tra ve bang tieng Viet.
    """
